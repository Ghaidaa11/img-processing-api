...
describe("test the api api/images", () => {
    const fullImagePath = path.join(process.cwd(), 'src', 'images', 'full', 'lion.jpg');
    ...
    it("the image is returned successfully", async () => {
        const res = await request(app).get('/api/images').query({filename: "lion", height: '250', width: '250'})
        expect(res.status).toBe(200)
    })

     it("the server is returning 500 if the image not found", async () => {
        const res = await request(app).get('/api/images').query({filename: "nothing", height: '250', width: '250'})
